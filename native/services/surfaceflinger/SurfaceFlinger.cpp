...
status_t SurfaceFlinger::readyToRun()
{
    ...
    //<<<< for GPU scaling. When using GPU scaling, the size of framebuffer
    //     unequal to the android visible size.
    mAppWidth = hw->setDispProp(DISPLAY_CMD_GETDISPPARA, 0, DISPLAY_APP_WIDTH, 0);
    mAppHeight = hw->setDispProp(DISPLAY_CMD_GETDISPPARA, 0, DISPLAY_APP_HEIGHT, 0);
    //>>>> for GPU scaling
...
void SurfaceFlinger::onVSyncReceived(int type, nsecs_t timestamp) {
    ...
    setDisplayProp(DISPLAY_CMD_POSTFB,0,0,0);
}
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    //<<<< for GPU scaling. When using GPU scaling, the size of framebuffer
    //     unequal to the android visible size.To scissor the unused field.
    const GLsizei valid_width = hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_VALID_WIDTH,0);
    if(mAppWidth > valid_width){
        glScissor(0, hw->getHeight() - mAppHeight, mAppWidth, hw->getHeight());
        glEnable(GL_SCISSOR_TEST);
    }
    //>>>> for GPU scaling.
...
int SurfaceFlinger::setDisplayParameter(uint32_t cmd,uint32_t  value)
{
    ...
    if (hwc.initCheck() == NO_ERROR) 
    {
        ...
        ret = hwc.setParameter(cmd,value);
        if(cmd == HWC_LAYER_SET3DMODE || (cmd == HWC_LAYER_SHOW && value == 0))
        {
            const sp<DisplayDevice>& hw(mDisplays[0]);
            screen_para_t screen_para;

            int cur_3denable = hwc.getParameter(HWC_LAYER_GET3DENABLE);
            hw->setDispProp(DISPLAY_CMD_SET3DMODE,0,cur_3denable,0);
