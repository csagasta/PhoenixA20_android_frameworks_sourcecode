...
MediaPlayerService::MediaPlayerService()
{
    ...
    /* add the extend data */
    {
        mGlobalExtend = new globalextenddata_t();
        if(mGlobalExtend == NULL)
            ALOGE("ERROR! fail in new globalextenddata_t().");
            
        globalextenddata_t *real_p = (globalextenddata_t *)mGlobalExtend;
        char prop_value[PROPERTY_VALUE_MAX];
        property_get(PROP_RAWDATA_KEY, prop_value, PROP_RAWDATA_DEFAULT_VALUE);
        ALOGV("raw data = %s", prop_value);
        String8 value(prop_value);
        if(value == PROP_RAWDATA_MODE_HDMI_RAW)
            real_p->rawDataMode = AUDIO_DATA_MODE_HDMI_RAW;
        else if(value == PROP_RAWDATA_MODE_SPDIF_RAW)
            real_p->rawDataMode = AUDIO_DATA_MODE_SPDIF_RAW;
        else
            real_p->rawDataMode = AUDIO_DATA_MODE_PCM;
    }
        ...
        int audioSessionId)
{
    ...
    /* 2013-3-16 */
    /* control the raw data mode */
    c->generalInterface(MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE, ((globalextenddata_t *)mGlobalExtend)->rawDataMode, 0, 0, 0);
        ...
        int audioSessionId, uid_t uid)
{
    ...
    /* 2013-3-16 */
    /* add the extend data */
    mExtend = new extenddata_t();
    if(mExtend == NULL)
        ALOGE("ERROR! fail in new extenddata_t().");
        
    extenddata_t *real_p = (extenddata_t *)mExtend;
    /* add by Gary. end   -----------------------------------}} */
...
sp<MediaPlayerBase> MediaPlayerService::Client::createPlayer(player_type playerType)
{
    ...
    if ((playerType == CEDARX_PLAYER) || (playerType == NU_PLAYER)){
        p->mMediaType = 1;
    } else {
        p->mMediaType = 0;
    } 
...
status_t MediaPlayerService::Client::setDataSource(
    ...
    /* 2013-3-16 */
    /* control the raw data mode */
    p->generalInterface(MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE, ((extenddata_t *)mExtend)->rawDataMode, 0, 0, 0);
    /* add by Gary. end   -----------------------------------}} */
        ...
        const sp<ISurfaceTexture>& surfaceTexture)
{
    ...
    if (surfaceTexture != NULL) {
        ...
        // To disconnect the native window make sure the native window
...
status_t MediaPlayerService::Client::prepareAsync()
{
    ...
    status_t ret; 
    int rotation_angle;
    rotation_angle = getRotation();

	if(rotation_angle)
	{
		//generalInterface(MEDIAPLAYER_CMD_SET_ROTATION, rotation_angle, 0, 0,NULL);
         if(0 == mHdmiPlugged)
        {
            ALOGD("(f:%s, l:%d) rotation_angle=%d, hdmi plugout, call generalInterface MEDIAPLAYER_CMD_SET_INIT_ROTATION", __FUNCTION__, __LINE__, rotation_angle);
            p->generalInterface(MEDIAPLAYER_CMD_SET_INIT_ROTATION, rotation_angle, 0, 0,NULL);
        }
	}
...
status_t MediaPlayerService::generalGlobalInterface(int cmd, int int1, int int2, int int3, void *p)
{
    switch(cmd){
		...
		case MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE:{
			ALOGD("enter MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE");
			status_t ret = OK;
			
			((globalextenddata_t *)mGlobalExtend)->rawDataMode = int1;
			ALOGE("++++++++++++++++++++++++++++++ %d", int1);
            if(int1 == AUDIO_DATA_MODE_HDMI_RAW)
                property_set(PROP_RAWDATA_KEY, PROP_RAWDATA_MODE_HDMI_RAW);
            else if(int1 == AUDIO_DATA_MODE_SPDIF_RAW)
                property_set(PROP_RAWDATA_KEY, PROP_RAWDATA_MODE_SPDIF_RAW);
            else
                property_set(PROP_RAWDATA_KEY, PROP_RAWDATA_MODE_PCM);
                
			for (int i = 0, n = mClients.size(); i < n; ++i) {
				sp<Client> c = mClients[i].promote();
				if (c != 0) {
					status_t temp = c->generalInterface(MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE, int1, int2, int3, p);
					if( temp != OK )
						ret = temp;
				}
			}
...
status_t MediaPlayerService::Client::generalInterface(int cmd, int int1, int int2, int int3, void *p)
{
    ...
    switch(cmd){
	case MEDIAPLAYER_GLOBAL_CMD_SET_RAWDATA_MODE:{
		((extenddata_t *)mExtend)->rawDataMode = int1;
        break;
    }
	default:
	    break;
