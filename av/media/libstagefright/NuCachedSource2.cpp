    ...
    if (reconnect) {
        ...
        if (err == ERROR_UNSUPPORTED || err == -EPIPE) {
            ...
            ALOGI("The attempt to reconnect failed, %d retries remaining",
                 mNumRetriesLeft);
			if (mNumRetriesLeft <= 1) {
				mNetWorkStatus = ERROR_IO;
			}
            return;
        }
    }
...
status_t NuCachedSource2::GetNetWorkStatus(int32_t *nNetworkState)
{
	*nNetworkState = mNetWorkStatus;
	return OK;
}
