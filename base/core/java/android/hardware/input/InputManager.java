        ...
        synchronized (mInputDevicesLock) {
            ...
            for (int i = 0; i < deviceIdAndGeneration.length; i += 2) {
                ...
                if (index >= 0) {
                    ...
                    IPowerManager ipm = IPowerManager.Stub.asInterface(b);
                    boolean isScreenOn = false;
                    try {
					     isScreenOn = ipm.isScreenOn();
					  } catch (RemoteException ex) {
					    Log.w(TAG, "Failed to get ScreenOn state.", ex);
						 }
										if( !isScreenOn )
										{
										 try {
							    ipm.wakeUp(android.os.SystemClock.uptimeMillis());
						 } catch (RemoteException ex) {
								 Log.w(TAG, "Failed to set  Screen On .", ex);
						 }
