...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    public WifiP2pService(Context context) {
        ...
        //set wfd p2p role
        SystemProperties.set(WFD_P2P_ROLE, "");//empty
        SystemProperties.set(WFD_DNSMASQ_PEER,"");//empty
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case WifiP2pManager.CONNECT:
                    ...
                    mWifiP2pDevice = getWifiP2pDeviceFromPeers(mSavedPeerConfig.deviceAddress);
                    //add end
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    if (mGroup.isGroupOwner()) {
                        ...
                        mWifiP2pDevice = getWifiP2pDeviceFromPeers(groupOwner.deviceAddress);
                        //add end
                        sendP2pPeersChangedBroadcast();
                    }
                    mSavedPeerConfig = null;
            ...
            switch (message.what) {
                case WifiMonitor.AP_STA_CONNECTED_EVENT:
                    ...
                    if (deviceAddress != null) {
                        ...
                        mWifiP2pDevice = getWifiP2pDeviceFromPeers(deviceAddress);
                        SystemProperties.set(WFD_P2P_ROLE,"1");//GO
                        //add end
                ...
                case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    ...
                    if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS &&
                        ...
                        SystemProperties.set(WFD_P2P_ROLE,"2");//client
                        String gateway = SystemProperties.get("dhcp.p2p.gateway");
                        if(gateway != null)
                            logd("gateway=["+gateway+"]");
                        else
                            loge("gateway not set in the property yet.");
                        String ipaddress = SystemProperties.get("dhcp.p2p.ipaddress");
                        if(ipaddress != null)
                            logd("ipaddress=["+ipaddress+"]");
                        else
                            loge("gateway not set in the property yet.");
                        sendP2pConnectionChangedBroadcast();
                ...
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    ...
                    if(ipaddress != null) {
                        ...
                        loge("ipaddress not set in the property yet.");
                    }

                    /*=== clear ip add start "p2p0" 2013-01-08 14:34 ===*/
                    logd("clear interface ip " + mInterface);
                    try {
                ...
                case WifiP2pManager.CONNECT:
                    ...
                    if (config.deviceAddress == null ||
                        ...
                        if (config.deviceAddress != null) {
                            ...
                            //add start
                            mWifiP2pDevice = getWifiP2pDeviceFromPeers(config.deviceAddress);
                            //add end
                            sendP2pPeersChangedBroadcast();
                        }
                        replyToMessage(message, WifiP2pManager.CONNECT_SUCCEEDED);
    ...
    private void sendP2pConnectionChangedBroadcast() {
        ...
        if(mWifiP2pDevice != null)
            intent.putExtra(WifiP2pManager.EXTRA_WIFI_P2P_DEVICE, new WifiP2pDevice(mWifiP2pDevice));
        else
...
/*
    ...
    private WifiP2pDevice getWifiP2pDeviceFromPeers(String deviceAddress) {
        for (WifiP2pDevice d : mPeers.getDeviceList()) {
            if(comparedMacAddr(d.deviceAddress, deviceAddress)){
                return d;
            }
        }
        return null;
	...
	private boolean comparedMacAddr(String inputa, String inputb){    	
		//42:fc:89:a8:96:09    	
