   ...
   /** @hide Maximum volume index values for audio streams */
            ...
            synchronized (streamState) {
                ...
                if (streamState.muteCount() != 0 && !isStreamAffectedByMute(streamType) &&
                    ...
                    for (int i = 0; i < size; i++) {
                        /*modified by chenjd, 2013-09-04
                        *fix bug, see class define of VolumeDeathHandler for detail
                        */
                        streamState.mMuteCount = 1;
    ...
    public class VolumeStreamState {
        ...
        */
        ...
        /*modified by chenjd, 2013-09-04
        *Fix AndroidBug:when one of the audio client enable mute, other audio client can not
        *  disable mute, or adjust volume.
        */
        private class VolumeDeathHandler implements IBinder.DeathRecipient {
            ...
            public void mute(boolean state) {
                if (state) {
                    if (mMuteCount == 0) {
                        ...
                        mMuteCount++;
                        Log.d(TAG, "stream: "+mStreamType+" is muted by ths client");
                    } else {
                        Log.w(TAG, "stream: "+mStreamType+" was already muted by ths/other client");
                    }
                } else {
                    if (mMuteCount == 0) {
			mDeathHandlers.remove(this);
				if (mICallback != null) {
                        ...
                        Log.e(TAG, "stream: "+mStreamType + " was already umute by ths/other client");
                    } else {
                        mMuteCount--;
                        if (mMuteCount == 0) {
        ...
        private VolumeDeathHandler getDeathHandler(IBinder cb, boolean state) {
			...
			Log.d(TAG, "haha, is the same Ibinder");
                    return handler;
                }
            }
            ...
            if (state) {
		Log.d(TAG, "a new handler===");
                handler = new VolumeDeathHandler(cb);
            } else {
                Log.w(TAG, "create a new umuted client");
                handler = new VolumeDeathHandler(cb);
                try{
			if(cb != null){
				cb.linkToDeath(handler, 0);
			}
			mDeathHandlers.add(handler);
                }catch (RemoteException e) {
			return null;
                }
