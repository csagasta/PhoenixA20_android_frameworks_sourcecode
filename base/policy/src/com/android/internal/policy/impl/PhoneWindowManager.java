...
/* add by Gary. end   -----------------------------------}} */
/*add by Chenjd. start  {{------------------------------*/
/*suport led flick when key pressing*/
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private static final char portType = 'd';
    ...
    private void interceptPowerKeyDown(boolean handled) {
        ...
        if (!handled) {
            ...
            long delay;
            boolean shortPressLeadShutdown = SystemProperties.getBoolean("ro.sw.shortpressleadshut", false);
            Log.d(TAG, "shortPressLeadShutdown = " + shortPressLeadShutdown);
            if(shortPressLeadShutdown)
                delay = 0;
            else
                delay = ViewConfiguration.getGlobalActionKeyTimeout() + 500;
            mHandler.postDelayed(mPowerLongPress, delay);
    ...
    private final Runnable mPowerLongPress = new Runnable() {
        ...
        public void run() {
            ...
            case LONG_PRESS_POWER_GLOBAL_ACTIONS:
                ...
                {
                    boolean directlyPowerOff = SystemProperties.getBoolean("ro.sw.directlypoweroff", false);
                    Log.d(TAG, "LONG_PRESS_POWER_GLOBAL_ACTIONS() :directlyPowerOff = " + directlyPowerOff);
                    if(directlyPowerOff)
            ...
            case LONG_PRESS_POWER_SHUT_OFF_NO_CONFIRM:
                ...
                {
                    boolean directlyPowerOff = SystemProperties.getBoolean("ro.sw.directlypoweroff", false);
                    Log.d(TAG, "LONG_PRESS_POWER_SHUT_OFF() :directlyPowerOff = " + directlyPowerOff);
                    if(directlyPowerOff)
                        mWindowManagerFuncs.shutdown(false);
                    else
...
//	    }
        ...
        initializeHdmiState();
        mFullScreenIsEnable = SystemProperties.getBoolean("ro.statusbar.inheritfullscn", true);
		mAlwaysFullScreen = SystemProperties.getBoolean("ro.statusbar.alwayshide", false);
		Log.d(TAG, "mFullScreenIsEnable = " + mFullScreenIsEnable);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        /* 20113-3-2 */
        /* flicker led when receive ir signal */
        long consumed = _interceptKeyBeforeDispatching(win, event, policyFlags);
    ...
    /* add by chenjd. end   -----------------------------------}} */
        ...
        /* 2011-11-1 */
        /* support the shortcut keys "setup", "expand" and "browser" */
        else if (keyCode == KeyEvent.KEYCODE_BROWSER || keyCode == KeyEvent.KEYCODE_EXPLORER) {
            Log.v(TAG, "it's KEYCODE_BROWSER key and down = " + down);
            if(!down){
                Uri uri = Uri.parse("http://www.qiyi.com");
        ...
        else if (keyCode == KeyEvent.KEYCODE_EXPAND) {
        ...
        /* support shortcut keys with color keys for some specific websites and apps */
            ...
            if(!down){
                ...
                ContentResolver resolver = mContext.getContentResolver();
                String str = Settings.System.getString(resolver, name);
                if(str == null){
                ...
                /* parse string */
                String[] strArray = str.split(Settings.System.SHORTCUT_PATH_SEPARATOR);
                if(strArray == null){
                    Log.w(TAG, "fail in spliting string " + str + " with substring \"--split--\".");
                    return -1;
                }
                Intent intent = null;
                if(strArray[0].equals(Settings.System.SHORTCUT_PATH_TYPE_WEBSITE)){
                    Uri uri = Uri.parse(strArray[1]);
                    intent = new Intent(Intent.ACTION_VIEW, uri);
                    intent.addCategory(Intent.CATEGORY_BROWSABLE);
                }else if(strArray[0].equals(Settings.System.SHORTCUT_PATH_TYPE_APP)){
                    intent = new Intent();
                    intent.setComponent(new ComponentName(strArray[1], strArray[2]));
                }
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(intent);
            }
        ...
        /* support shortcut keys : movie and apps */
            ...
            if(!down){
                Intent intent = new Intent();
                intent.setComponent(new ComponentName("com.softwinner.launcher", "com.softwinner.launcher.Launcher"));
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        if (isDefaultDisplay) {
            ...
            boolean navVisible = (((mLastSystemUiFlags & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0) && (mTopIsFullscreen == false))||(mFullScreenIsEnable == false);
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_FULLSCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (attrs.type == TYPE_STATUS_BAR_PANEL
                    ...
                    {	                	
						...
						if (DEBUG_LAYOUT) {
                        Log.v(TAG, String.format(
                                    "Laying out IN_SCREEN5 status bar window: (%d,%d - %d,%d)",
                                    pf.left, pf.top, pf.right, pf.bottom));
                    }
					}					
					else					
					{	                    
						...
						if (DEBUG_LAYOUT) {
                        Log.v(TAG, String.format(
                                    "Laying out IN_SCREEN6 status bar window: (%d,%d - %d,%d)",
                                    pf.left, pf.top, pf.right, pf.bottom));
                    }
					}
                }
    ...
    public int finishPostLayoutPolicyLw() {
		...
		{
			if (DEBUG_LAYOUT) Log.i(TAG, "mNavigationBar force=" + mForceStatusBar
                    + " top=" + mTopFullscreenOpaqueWindowState);
            if (mForceStatusBar) {
                if (DEBUG_LAYOUT) Log.v(TAG, "Showing status bar: forced");
	...
	try{
		return audioService.isStreamMute(AudioManager.STREAM_MUSIC);
	}catch (RemoteException e){
		return false;
	}
    }
    ...
    void handleMute(){
        ...
        try {
            ...
            // that the music stops right before we call this
            mBroadcastWakeLock.acquire();
            audioService.setStreamMute(AudioManager.STREAM_MUSIC, !isMute(), mCallback);
        } catch (RemoteException e) {
            Log.w(TAG, "IAudioService.adjustStreamVolume() threw RemoteException " + e);
        } finally {
            mBroadcastWakeLock.release();
