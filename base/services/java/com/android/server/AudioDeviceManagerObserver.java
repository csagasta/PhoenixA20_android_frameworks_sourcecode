...
public class AudioDeviceManagerObserver extends UEventObserver {
    ...
    private void startObservingTv(Context context){
        ...
        BroadcastReceiver receiver = new BroadcastReceiver(){
            public void onReceive(Context context, Intent intent){
                ...
                if (action.equals(Intent.ACTION_HDMISTATUS_CHANGED))
                {
                    ...
                    if(hdmiplug == 1){
                    	Log.d(TAG, "hdmiplug == 1");
                    	Log.d(TAG, "getDisplayOutputType = "+mDisplayManager.getDisplayOutputType(0));
                        if(mDisplayManager.getDisplayOutputType(0) == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_HDMI || mDisplayManager.getDisplayOutputType(0) == 0){
        ...
        if(curType == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_HDMI && getHdmiPlugState() == PLUG_IN){
            Log.d(TAG, "curType == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_HDMI && getHdmiPlugState() == PLUG_IN\n");
            notifyHdmiAvailable();
        }
    }
    ...
    private void notifyHdmiAvailable(){
        ...
        if(AudioManager.getHdmiExpected()){
        	Log.d(TAG, "setAudioDeviceActive\n");
